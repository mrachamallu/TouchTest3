...
class DrawingView2: UIView {
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        let distFromOrigin = sqrt(pow(Double(lastPoint.x) - centerX, 2) + pow(Double(lastPoint.y) - centerY, 2))
        //var curveXCoord = sqrt(pow(RADIUS, 2) - pow(Double(lastPoint.y) - centerY, 2)) + centerX
        //var curveYCoord = sqrt(pow(RADIUS, 2) - pow(Double(lastPoint.x) - centerX, 2)) + centerY
       
        //if the point is too close to the center or too far from the center, it is out of bounds
